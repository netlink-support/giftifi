...
$(".fintech-queries-form").each(function () {
    ...
    $(this).validate({
        ...
        rules: {
            first_name: { required: true, noSpaces: true },
            last_name: { required: true, noSpaces: true },
            email: { required: true, email: true },
            phone: { required: true, digits: true, minlength: 10 },
            interest: { required: true },
            organization: { required: true },
            city: { required: true, noSpaces: true }
        },
            ...
            grecaptcha.enterprise.execute('6LfoRqAqAAAAAK3G1RcmDp09XV0y9b1P-ISPgpvn', { action: 'submit' })
                .then(function (token) {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: base_url + 'ajax/',
                        data: $(form).serialize() + '&parent_url=' + window.location.href,
                        dataType: 'json',
                        ...
                        error: function () {
                            $('#brochure_form_response')
                                .html('<p class="text-danger fw-bold">Error submitting form. Please try again.</p>');
                        }
                    });
